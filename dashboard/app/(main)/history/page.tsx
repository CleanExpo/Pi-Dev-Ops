...
const TRIGGER_COLOR = { manual: "var(--text-muted)", webhook: "var(--accent)", cron: "#4ADE80" };
const STATUS_COLOR  = { running: "var(--accent)", done: "#4ADE80", error: "#F87171" };

function QualityScore({ result }: { result: Partial<AnalysisResult> | null }) {
  const q = result?.quality;
  if (!q) return <span className="font-mono text-xs" style={{ color: "var(--text-dim)" }}>—</span>;
              ...
              <tr key={s.id} style={{ borderBottom: "1px solid var(--border)" }}
                ...
                <td className="px-4 py-2">
                  <span className="font-mono text-[10px] uppercase"
                    style={{ color: TRIGGER_COLOR[s.trigger] ?? "var(--text-dim)" }}>
                    {s.trigger}
                  </span>
                </td>
                <td className="px-4 py-2">
                ...
                <td className="px-4 py-2">
                  {s.pr_url
                    ? <a href={s.pr_url} target="_blank" rel="noopener noreferrer"
                        className="font-mono text-[10px] hover:opacity-70" style={{ color: "var(--accent)" }}>
                        OPEN ↗
                      </a>
          ...
          <div key={s.id} className="px-3 py-3 flex flex-col gap-2">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              <div className="flex items-center gap-1">
                <span className="font-mono text-[10px]" style={{ color: "var(--text-dim)" }}>Q:</span>
                <QualityScore result={s.result} />
              </div>
              <div className="flex items-center gap-1">
                <span className="font-mono text-[10px]" style={{ color: "var(--text-dim)" }}>ZTE:</span>
                <span className="font-mono text-xs" style={{ color: "var(--text)" }}>
                  {s.result?.zteScore !== undefined ? `${s.result.zteScore}/60` : "—"}
                </span>
              </div>
