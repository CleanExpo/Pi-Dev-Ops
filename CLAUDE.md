...
- **Codebase wiki (RA-1968):** self-updating per-directory `WIKI.md` via post-merge `.github/workflows/codebase-wiki.yml` → `scripts/run_codebase_wiki.py --since=<sha>` → `tao_codebase_wiki.update_wiki(...)`; token-budget guard (default `max_cost_usd=0.02`/dir; Action sets `TAO_MAX_COST_USD=0.05`); `LoopCounter.tick()` per dir; `concurrency: codebase-wiki` cancel-in-progress; `if: github.repository == 'CleanExpo/Pi-Dev-Ops'`.
  - **Working with the wiki (session convention):** start in a directory by reading its `WIKI.md` for orientation (recent changes + intent); `WIKI.md` (root) indexes all per-directory wikis. After substantive changes on a feature branch, refresh locally — `python3 scripts/run_codebase_wiki.py --since=<base-sha> --directories=<dir>` (drop `--dry-run` to write; the post-merge Action handles `main` automatically). Treat `WIKI.md` as derived: never hand-edit a per-directory `WIKI.md` — change the code and regenerate.
