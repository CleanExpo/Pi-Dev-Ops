...
- **TAO context compactor (RA-1967):** Deterministic, LLM-free transcript compactor — port of `@sting8k/pi-vcc`. Public API `compact(messages, target_token_budget=None) -> (messages, CompactionStats)` lives in `app/server/tao_context_vcc.py`. Four passes (whitespace normalise, repeat-pattern collapse, tool-output dedup, verbose-block head/tail truncation) — all pure, idempotent, zero token cost. SKILL: `skills/tao-context-vcc/`. Wired as the deferred `compact_for_sdk(messages)` helper rather than a hard pre-pass on `_run_claude_via_sdk` (TODO comment in-source — RA-1970 will resolve once `tao-loop` settles the canonical message representation). Manual verification (canonical, RA-1967b): `python3 scripts/validate_tao_context_vcc.py --root ~/.claude/projects -n 10` runs `compact()` over the top-N largest real Claude Code conversation jsonl sessions and exits 0 when median token reduction ≥ 30%. Threshold per the 2026-05-05 board memo. Validator auto-detects record shape — both `~/.claude/projects/**/*.jsonl` (full transcripts) and `.harness/agent-sdk-metrics/*.jsonl` (SDK metadata, no message body — emits skip notice) are accepted.
...
- **Context mode (RA-1969, Wave 1):** port of `context-mode`. Build a `CodebaseIndex` once via `app.server.tao_context_mode.build_index(repo_root)` (~200B/file synopsis + symbols + sha256), then `expand(index, path)` only the files actually needed. No LLM calls — pure regex + first-comment synopsis, deterministic given file contents. Kill-switch tick per file. SKILL: `skills/tao-context-mode/`. CLI: `python scripts/run_tao_context_mode.py --repo-root .`. Validation: `python scripts/validate_tao_context_mode.py --repo-root .` compares **context-mode vs vcc baseline** (NOT raw harness — vcc already gives ~56%); board-memo threshold is median ≥40% additional reduction. Per the 2026-05-05 board verdict, missing the threshold is WATCH not REJECT; experiment log gates merge.
