...
**What it is:** The founder wants the command console visual (graphs/3D) + voice, not text. Source: [execution-gap note](../plaud/2026-06-10-06-11-bridging-the-execution-gap-in-an-ai-driven-business-ecosystem-a-systemic-overhaul-mandate.md) (Mission Station / Nexus). **Routing flag — RESOLVED 2026-07-03:** brand ownership confirmed by Phill: Unite-Group Nexus Pty Ltd owns the ecosystem; Duncan Perkins is a client (Home Loan Essentials / ITR-Button), not the Nexus owner or namesake. `NEXUS.md` corrected accordingly. Running code stays routed to pi-dev-ops (`app/server/routes/mission_control.py`, `dashboard/`).
...
**Effort/risk:** L / Med. **Dependencies:** `ELEVENLABS_API_KEY` (already used by the 6-pager). **Open question:** ~~pi-dev-ops vs unite-group ownership~~ resolved — see routing flag above.
