...
Before using any host `worktree` command, prove the intended host target is a real
Git checkout. The historic `~/Pi-CEO/Pi-Dev-Ops` directory is an operational-data
wrapper, not a repository; never initialise Git there and never ask a host to enter
an existing worktree from there.

Use the handoff's declared worktree, or the authoritative repository root, with
the installed launcher:

```bash
pi-ceo-harness --host claude --worktree "<handoff worktree>" --check
pi-ceo-harness --host codex --worktree "<handoff worktree>" --check
```

If it reports `ENTRY_READY`, restart the Claude/Codex session through the same
launcher and matching `--host` before resuming. `ENTRY_RECOVERED` means the legacy
wrapper was safely redirected to the configured real checkout. `ENTRY_BLOCKED` is a
hard stop: record the supplied path and use the handoff's exact checkout rather
than guessing.
