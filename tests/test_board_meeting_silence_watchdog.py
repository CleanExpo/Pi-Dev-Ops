...
@pytest.mark.asyncio
async def test_unhashable_status_record_does_not_crash_watchdog(
    isolated_success_dir, isolated_meetings_dir, _hush_network, caplog
):
    """Codex round-3 P1 regression — a record whose status is an unhashable
    JSON array/object must not raise TypeError out of the watchdog. The record
    is rejected -> the watchdog completes and alerts."""
    (isolated_success_dir / f"{_BOARD_MEETING_JOB_ID}.json").write_text(
        '{"job_id": "' + _BOARD_MEETING_JOB_ID + '", "status": [], "ts": 0}',
        encoding="utf-8",
    )
    with caplog.at_level(logging.WARNING, logger="pi-ceo"):
        await cw._watchdog_board_meeting_silence(logging.getLogger("pi-ceo"))
    assert _alerted(caplog), "an uninterpretable status must not mask silence"
    assert cw._board_meeting_silent_last_raised > 0
