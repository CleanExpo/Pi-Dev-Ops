...
async def test_plan_failures_block_without_advancing_checkpoint(
    ...
    """All planner failure modes persist BLOCKED and leave plan incomplete."""
...
async def test_run_build_never_starts_generator_when_planner_blocks(tmp_path):
    """The pipeline must stop even if a blocked planner is replaced or retried."""
    ...
    with patch.object(session_phases, "require_active_for_run"), \
         patch.object(session_phases, "em"), \
