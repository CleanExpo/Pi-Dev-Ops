...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("cheap", 1, "mid", "delegate"),
        ("cheap", 2, "top", "delegate"),
        ("cheap", 3, "top", "bailout"),
        ("mid", 0, "mid", "delegate"),
        ("mid", 1, "top", "delegate"),
        ("mid", 2, "top", "bailout"),
...
def test_comparable_failures_follow_the_terminal_escalation_chain(
    ...
    assert ("monotonic-failure-escalation" in decision.reasons) is bool(prior_failures)
...
def test_exhausted_top_route_has_terminal_non_dispatch_semantics() -> None:
    decision = decide_route(_request(task_shape="top", prior_failures=1))
