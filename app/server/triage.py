...
class TriageEngine:
    ...
    def triage(
        ...
        for result, finding in all_findings:
            ...
            try:
                ...
                # RA-586 — critical findings → Telegram alert (< 5 min SLA)
                if finding.severity == "critical":
                    self._telegram_alert(result.project_id, finding, issue)
    ...
    def _telegram_alert(self, project_id: str, finding: Finding, issue: dict[str, Any]) -> None:
        """
        RA-586 — Send a Telegram message to Phill for every critical Pi-SEO finding.
        Reads TELEGRAM_BOT_TOKEN and TELEGRAM_ALERT_CHAT_ID from config.
        Silently skips if either is unset (non-fatal; Linear ticket already created).
        """
        import app.server.config as _cfg
        token = _cfg.TELEGRAM_BOT_TOKEN
        chat_id = _cfg.TELEGRAM_ALERT_CHAT_ID
        if not token or not chat_id:
            log.debug("Telegram alert skipped — token or chat_id not configured")
            return
        ...
        text = (
            ...
            f"_{finding.description[:200]}{'…' if len(finding.description) > 200 else ''}_"
        )
        ...
        try:
            ...
            if result.get("ok"):
                log.info("Telegram alert sent for critical finding in %s (%s)", project_id, identifier)
                telegram_sent = True
            else:
                log.warning("Telegram sendMessage returned ok=false: %s", result)
        except Exception as exc:
            log.warning("Telegram alert failed for %s: %s (non-fatal)", identifier, exc)

        # RA-633 — log alert to Supabase for escalation watchdog
        try:
            from .supabase_log import log_alert_escalation as _log_alert_esc
            _log_alert_esc(
                alert_key=finding.fingerprint,
            ...
            log.warning("alert_escalation Supabase log failed (non-fatal): %s", exc)
