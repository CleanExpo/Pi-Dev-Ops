...
def models_for_lane(lane: str) -> list[str]:
    ...
    models = (
        ...
        if raw
        else list(DEFAULT_LANE_MODELS.get(lane, ()))
    )
...
def strength_model() -> str:
    value = (
        os.environ.get("OMNIROUTE_MODEL_STRENGTHEN")
        or DEFAULT_STRENGTH_MODEL
    ).strip()
...
def _remember(call: FabricCall) -> None:
    ...
    global _total_fallbacks, _total_strengthened
    with _lock:
        _last_call = call
        _total_calls += 1
        if not call.ok:
    ...
    *,
    ...
    try:
        ...
        return (
            ...
            f"omniroute_http_{exc.code}:{body}",
        )
    ...
    if _is_banned_model(served_model) or _is_banned_model(provider):
        return (
            ...
            provider,
            latency_ms,
            f"omniroute_served_banned_route:{provider}:{served_model}",
        )
    if not text:
        return (
            False,
...
def _should_strengthen(role: str, prompt: str) -> bool:
    if role != "margot.casual":
        return False
    mode = (
        os.environ.get("OMNIROUTE_STRENGTHEN_MARGOT") or "smart"
    ).strip().lower()
...
def _post_slack_trace(call: FabricCall) -> None:
    ...
    if not token or not channel:
        return
    attempts = ", ".join(f"`{x}`" for x in call.attempts)
    text = (
        "*Margot Model Fabric*\n"
        f"lane: `{call.lane}` · served: `{call.served_model or 'unknown'}` · "
        f"provider: `{call.provider or 'unknown'}` · {call.latency_ms}ms\n"
        f"attempts: {attempts}\n"
        f"strengthened: {'yes' if call.strengthened else 'no'} · "
        f"result: {'PASS' if call.ok else 'FAIL'}"
    )
    req = urllib.request.Request(
...
def complete(
    *,
    ...
    if not draft:
        call = FabricCall(
            time.time(), role, lane, attempts[0], served_model, provider,
            total_latency, False, attempts, False, last_error,
        )
        _remember(call)
    ...
    if _should_strengthen(role, prompt):
        ...
        if reviewer and reviewer != served_model:
            ...
            ok, text, served, prov, latency_ms, error = _chat_once(
                ...
                model=reviewer,
                session_id=(
                    f"{session_id}-strength" if session_id else "margot-strength"
                ),
                timeout_s=timeout_s,
                max_tokens=max_tokens,
            )
    ...
    call = FabricCall(
        time.time(), role, lane, models[0], served_model, provider,
        total_latency, True, attempts, strengthened, None,
    )
    _remember(call)
    _post_slack_trace(call)
    log.info(
        "model_fabric role=%s lane=%s served=%s provider=%s attempts=%d "
        "strengthened=%s latency_ms=%d",
        role,
        lane,
        served_model,
        provider or "unknown",
...
def status_snapshot() -> dict[str, Any]:
    ping_ok, ping = (
        _get_json("/api/health/ping") if enabled() else (False, "disabled")
    )
    ...
    return {
        ...
        "lanes": {
            lane: {
                ...
                "banned": any(
                    _is_banned_model(model) for model in models_for_lane(lane)
                ),
            }
