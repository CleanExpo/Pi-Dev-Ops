...
| 3 | Optional: set `SUPABASE_UNITE_GROUP_URL` + `SUPABASE_UNITE_GROUP_SERVICE_KEY` on Vercel | Vercel | Direct Vercel→Supabase wiki reads skip the Railway hop. Not required for a green `cc-wiki-graph` probe: the dashboard now accepts the Railway aliases `UGO_SUPABASE_*` and otherwise proxies `GET /api/wiki-graph` on Railway, then returns a stable empty graph at 200 if both hops have no key. |
...
Railway already has the same project as `UGO_SUPABASE_URL` / `UGO_SUPABASE_SERVICE_KEY`. The
dashboard wiki-graph route accepts those aliases, then asks Railway `GET /api/wiki-graph` if
Vercel still has no key, then returns a stable empty graph at 200. Setting the Vercel pair is
optional: it lets Vercel read `wiki_pages` without the Railway hop.

**Do not "fix" a remaining 503 by relaxing the smoke expectation to 503.** `.github/smoke-surfaces.json`
expects 200 + `pageCount`. A green check that accepted 503 would assert the feature is absent.

Verify with **Smoke Test E2E (production)** `mode=horizontal` and the `cc-wiki-graph` line. It
should go `200` with a `pageCount` (0 is a valid empty graph).
